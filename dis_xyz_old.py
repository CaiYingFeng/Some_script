...
eval_name='fc_50to1_half'
gt_name1='0711_front_center20to1_align2'
gt_name='0808_front_center50to1_half'
#评估位移用c2w
gt_path='/media/autolab/disk_4T/cyf/localization/out/eval/aachen/'+gt_name1+'.txt'
...
#评估角度用w2c
# gt_path='/media/autolab/disk_4T/cyf/localization/out/eval/aachen/'+gt_name+'_w2c.txt'#w2c:name qw,qx,qy,qz,tx,ty,tz
# estimate_path='/media/autolab/disk_4T/cyf/localization/out/eval/aachen/'+eval_name+'/'+eval_name+'_poses.txt'
f=open(gt_path)
gt=list(f)
...
for i in range(len(gt)):
    a1=gt[i].split(' ')
    a2=estimate[i].split(' ')
    # angle1=[float(a1[1]),float(a1[2]),float(a1[3]),float(a1[4])]
    # angle2=[float(a2[1]),float(a2[2]),float(a2[3]),float(a2[4])]
